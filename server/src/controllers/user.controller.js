...
async function registerUser(req, res) {
    ...
    if (!userName || !email || !password || !firstName) {
        return res
            .status(400)
            .json(
                new apiResponse((400, {}, "username, email or password not provided"))
            );
    }
    ...
    try {
        ...
        res
            ...
            .cookie("refreshToken", refreshToken, cookieConfig)
            .json(
                new apiResponse(200, { user: resData }, "User Registered succesfully")
            );
    } catch (e) {
...
async function updateUserName(req, res) {
    ...
    if (!newUserName) {
        return res
            .status(400)
            .json(new apiResponse(400, {}, "username not provided"));
    }
    try {
...
async function updatePassword(req, res) {
    ...
    if (!newPassword) {
        return res
            .status(400)
            .json(new apiResponse(400, {}, "password not provided"));
    }
    try {
        let user = await User.findById(req.user._id);
